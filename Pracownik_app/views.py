...
def getSearch(request, bookName=""):
    if request.method=='GET':
        # DO WPISANIA ISBN 10 KSIĄŻEK,
        if bookName != "":
            ksiazki = Ksiazka.objects.filter(Q(TytulKs__contains=bookName))

            ksiazka_serializer = KsiazkaSerializer(ksiazki,many=True)
...
def getBestsellers(request):
    if request.method=='GET':
        # DO WPISANIA ISBN 10 KSIĄŻEK,
        ksiazki = Ksiazka.objects.filter(ISBN__in = set([
            "9788377743798", 
            "9788327713971",
            "9788320824797", 
            "1942192419241",
...
@api_view(['GET'])
@parser_classes([JSONParser])
def getPromoBook(request):
    if request.method=='GET':
        #   ISBN DO WPISANIA
        ksiazka = Ksiazka.objects.get(ISBN="9788381255844")
        ksiazka_serializer = KsiazkaSerializer(ksiazka,many=False)
        # PRZECENA (W %%) DO WPISANIA
     #   promoJSON = {"ilePrzecena":"50"}
     #   promoJSON = json.dumps(promoJSON)
     #   promoJSON = json.dumps(promoJSON, ksiazka_serializer.data)
        return JsonResponse(ksiazka_serializer.data,safe=False)
...
def getSixNewestBooksApi(request):
    if request.method=='GET':
        ...
        ksiazka_serializer = KsiazkaSerializer(ksiazka[len(ksiazka)-6:],many=True)
        return JsonResponse(ksiazka_serializer.data,safe=False)
